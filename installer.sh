...
case $platform in
    ...
    *)
        ...
        echo "Please open an issue: https://github.com/prnvbn/bus/issues/new"
        exit 1
    ;;
esac
...
download_url=$(curl -s https://api.github.com/repos/prnvbn/bus/releases/latest | 
...
curl -sL "$download_url" -o bus
chmod +x bus

echo "-------------------------------------------------------------------"
echo "✅ Bus has been downloaded to: $(pwd)/bus"
echo ""

# Ask where to move the binary (loop until valid input)
while true; do
    echo "Where would you like to install Bus?"
    ...
    case "$choice" in
        1)
            sudo mv bus /usr/local/bin/
            echo "✅ Bus has been installed globally! You can now run 'bus' from anywhere."
            break
        ;;
        2)
            mkdir -p "$HOME/.local/bin"
            mv bus "$HOME/.local/bin/"
            echo "✅ Bus has been installed to ~/.local/bin."
        ...
        ;;
        3)
            echo "ℹ️  Bus will remain in the current directory."
...
echo "   https://github.com/prnvbn/bus/tree/main?tab=readme-ov-file#enabling-command-autocompletion"
echo "-------------------------------------------------------------------"